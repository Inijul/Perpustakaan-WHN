<style>
    /* Glass morphism effect untuk button logout */
    .logout-btn {
        position: relative;
        overflow: hidden;
    }
...
{{-- Sidebar --}}
<div
    class="w-[300] flex-shrink-0 bg-[#135ea2] p-8 md:p-[30px] flex flex-col items-center justify-start overflow-y-auto shadow-2xl">
    ...
    <div class="flex flex-col items-center gap-10 md:gap-20"> {{-- Kurangi gap untuk mobile --}}
        ...
        <svg width="121" height="120" viewBox="0 0 121 120" fill="none" xmlns="http://www.w3.org/2000/svg"
            class="w-[80px] h-[80px] md:w-[100px] md:h-[100px] transition-transform duration-300 hover:scale-110" preserveAspectRatio="xMidYMid meet">
        ...
        <div class="flex flex-col items-start gap-8 md:gap-[50px]"> {{-- Kurangi gap untuk mobile --}}
            <div class="flex flex-col items-start gap-8 md:gap-10"> {{-- Kurangi gap untuk mobile --}}
                {{-- Dashboard Link --}}
                <a href="/dashboard"
                    class="glass-effect group flex justify-start items-center h-[44px] w-full sm:w-[200px] pl-4 pr-2 border border-transparent gap-4 md:gap-5 rounded-2xl transition-all duration-300 ease-in-out hover:bg-[#10487a] hover:scale-105 hover:shadow-lg {{ request()->is('dashboard') || request()->is('/dashboard') || request()->is('/') ? 'bg-[#10487a] shadow-lg scale-105' : '' }}">
                    {{-- Kurangi gap --}}
                    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg"
                        class="w-6 h-6 md:w-[30px] md:h-[30px] flex-shrink-0 transition-transform duration-300 group-hover:scale-110" preserveAspectRatio="xMidYMid meet"> {{--
                ...
                {{-- Kelola Koleksi Link --}}
                <a href="koleksi"
                    class="glass-effect group flex justify-start items-center h-[44px] w-full sm:w-[200px] pl-4 pr-2 border border-transparent gap-4 md:gap-5 rounded-2xl transition-all duration-300 ease-in-out hover:bg-[#10487a] hover:scale-105 hover:shadow-lg @if(request()->is('koleksi')) bg-[#10487a] shadow-lg scale-105 @endif">
                    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg"
                        class="w-6 h-6 md:w-[30px] md:h-[30px] flex-shrink-0 transition-transform duration-300 group-hover:scale-110" preserveAspectRatio="xMidYMid meet">
                        <path
                ...
                {{-- Aktivitas Link --}}
                <a href="aktivitas"
                    class="glass-effect group flex justify-start items-center h-[44px] w-full sm:w-[200px] pl-4 pr-2 border border-transparent gap-4 md:gap-5 rounded-2xl transition-all duration-300 ease-in-out hover:bg-[#10487a] hover:scale-105 hover:shadow-lg @if(request()->is('aktivitas')) bg-[#10487a] shadow-lg scale-105 @endif">
            ...
            {{-- Logout Button --}}
            <button type="button" onclick="showLogoutModal()" class="glass-effect group flex justify-start items-center h-[44px] w-full sm:w-[200px] pl-4 pr-2 border border-transparent gap-4 md:gap-5 hover:bg-[#10487a] rounded-2xl transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-lg cursor-pointer">
                    <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg"
                        class="w-6 h-6 md:w-[30px] md:h-[30px] flex-shrink-0 transition-transform duration-300 group-hover:scale-110" preserveAspectRatio="xMidYMid meet">
                        <path
...
<div id="logout-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" style="display: none;">
    <div class="bg-white rounded-[30px] w-full max-w-sm mx-auto relative border border-gray-200 shadow-2xl transform transition-all duration-300 ease-out scale-95 opacity-0" id="logout-modal-content">
        <!-- Modal Header -->
        ...
        <div class="px-6 pb-4 text-center">
            <h3 class="text-xl font-bold text-gray-900">Anda yakin ingin logout?</h3>
        </div>
        ...
        <div class="flex justify-between gap-4 px-6 pb-6">
            ...
            <form action="{{ route('logout') }}" method="POST" class="flex-1">
                @csrf
                <button type="submit" 
                        class="glass-effect w-full bg-red-600 text-white rounded-[30px] py-2 font-medium hover:bg-red-700 transition-all duration-300">
                    Ya, Logout
                </button>
            </form>
        </div>
